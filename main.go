package main

import (
	"encoding/json"
	"fmt"
	"huy/evolutions"
	"huy/skim_monsters"
	"io/ioutil"
	"log"
)

type Pokemon struct {
	Skim *skim_monsters.Data
	Evol *evolutions.Data
}

func GetMonsterByID(id string) *Pokemon {
	pathFile := fmt.Sprintf("./skim_monsters/data/%s.json", id)
	file, err := ioutil.ReadFile(pathFile)
	if err != nil {
		log.Fatalf("Failed to read file: %s\nError: %s", pathFile, err)
	}
	var monster skim_monsters.Data
	err = json.Unmarshal(file, &monster)
	if err != nil {
		log.Fatalf("Failed to unmarshal file: %s\nError: %s", pathFile, err)
	}

	pathFile = fmt.Sprintf("./evolutions/data/%s.json", id)
	file, err = ioutil.ReadFile(pathFile)
	if err != nil {
		log.Fatalf("Failed to read file: %s\nError: %s", pathFile, err)
	}
	var evol evolutions.Data
	err = json.Unmarshal(file, &evol)
	if err != nil {
		log.Fatalf("Failed to unmarshal file: %s\nError: %s", pathFile, err)
	}

	return &Pokemon{
		Skim: &monster,
		Evol: &evol,
	}
}

func main() {
	var pokemons = map[string]string{
		"Deoxys-normal":        "386",
		"Munna":                "517",
		"Tranquill":            "520",
		"Duosion":              "578",
		"Mienfoo":              "619",
		"Seel":                 "86",
		"Medicham":             "308",
		"Relicanth":            "369",
		"Cyndaquil":            "155",
		"Stantler":             "234",
		"Musharna":             "518",
		"Shinx":                "403",
		"Conkeldurr":           "534",
		"Kyurem":               "646",
		"Primeape":             "57",
		"Nincada":              "290",
		"Monferno":             "391",
		"Meditite":             "307",
		"Vespiquen":            "416",
		"Dwebble":              "557",
		"Cranidos":             "408",
		"Emolga":               "587",
		"Nidoking":             "34",
		"Crobat":               "169",
		"Ho-oh":                "250",
		"Numel":                "322",
		"Vullaby":              "629",
		"Slowpoke":             "79",
		"Phanpy":               "231",
		"Wailord":              "321",
		"Arceus":               "493",
		"Seismitoad":           "537",
		"Slowking":             "199",
		"Girafarig":            "203",
		"Snorunt":              "361",
		"Scraggy":              "559",
		"Frillish":             "592",
		"Charmeleon":           "5",
		"Staraptor":            "398",
		"Dialga":               "483",
		"Feebas":               "349",
		"Pelipper":             "279",
		"Breloom":              "286",
		"Claydol":              "344",
		"Magby":                "240",
		"Seviper":              "336",
		"Gible":                "443",
		"Cubone":               "104",
		"Shaymin-land":         "492",
		"Kyogre":               "382",
		"Cherrim":              "421",
		"Rotom":                "479",
		"Mewtwo":               "150",
		"Furret":               "162",
		"Ledian":               "166",
		"Beldum":               "374",
		"Gurdurr":              "533",
		"Axew":                 "610",
		"Voltorb":              "100",
		"Swalot":               "317",
		"Carvanha":             "318",
		"Mawile":               "303",
		"Crawdaunt":            "342",
		"Skorupi":              "451",
		"Drapion":              "452",
		"Yanmega":              "469",
		"Cloyster":             "91",
		"Slugma":               "218",
		"Wingull":              "278",
		"Trubbish":             "568",
		"Garchomp":             "445",
		"Pidove":               "519",
		"Solosis":              "577",
		"Larvitar":             "246",
		"Azurill":              "298",
		"Duskull":              "355",
		"Gallade":              "475",
		"Shelmet":              "616",
		"Snubbull":             "209",
		"Shelgon":              "372",
		"Drifloon":             "425",
		"Staravia":             "397",
		"Mienshao":             "620",
		"Oddish":               "43",
		"Alakazam":             "65",
		"Sentret":              "161",
		"Persian":              "53",
		"Unown":                "201",
		"Swinub":               "220",
		"Smeargle":             "235",
		"Infernape":            "392",
		"Lucario":              "448",
		"Venonat":              "48",
		"Snorlax":              "143",
		"Noctowl":              "164",
		"Dusknoir":             "477",
		"Elgyem":               "605",
		"Armaldo":              "348",
		"Metang":               "375",
		"Combee":               "415",
		"Bisharp":              "625",
		"Arbok":                "24",
		"Poliwhirl":            "61",
		"Tyranitar":            "248",
		"Forretress":           "205",
		"Lileep":               "345",
		"Torterra":             "389",
		"Spiritomb":            "442",
		"Nidoqueen":            "31",
		"Dragonite":            "149",
		"Jumpluff":             "189",
		"Spearow":              "21",
		"Growlithe":            "58",
		"Beartic":              "614",
		"Altaria":              "334",
		"Leafeon":              "470",
		"Yamask":               "562",
		"Reuniclus":            "579",
		"Galvantula":           "596",
		"Clefable":             "36",
		"Ninetales":            "38",
		"Flaaffy":              "180",
		"Cubchoo":              "613",
		"Bulbasaur":            "1",
		"Metagross":            "376",
		"Glameow":              "431",
		"Murkrow":              "198",
		"Prinplup":             "394",
		"Togekiss":             "468",
		"Palkia":               "484",
		"Timburr":              "532",
		"Raticate":             "20",
		"Flareon":              "136",
		"Igglybuff":            "174",
		"Sawsbuck":             "586",
		"Heracross":            "214",
		"Cacnea":               "331",
		"Swellow":              "277",
		"Throh":                "538",
		"Nidorino":             "33",
		"Teddiursa":            "216",
		"Sceptile":             "254",
		"Magmortar":            "467",
		"Audino":               "531",
		"Eelektrik":            "603",
		"Durant":               "632",
		"Gastly":               "92",
		"Yanma":                "193",
		"Shuppet":              "353",
		"Gardevoir":            "282",
		"Rayquaza":             "384",
		"Shieldon":             "410",
		"Kakuna":               "14",
		"Hoppip":               "187",
		"Shuckle":              "213",
		"Sharpedo":             "319",
		"Spinda":               "327",
		"Toxicroak":            "454",
		"Gothita":              "574",
		"Weepinbell":           "70",
		"Geodude":              "74",
		"Kabutops":             "141",
		"Ludicolo":             "272",
		"Kecleon":              "352",
		"Accelgor":             "617",
		"Wigglytuff":           "40",
		"Tentacool":            "72",
		"Qwilfish":             "211",
		"Magnemite":            "81",
		"Slaking":              "289",
		"Amoonguss":            "591",
		"Lapras":               "131",
		"Typhlosion":           "157",
		"Manaphy":              "490",
		"Lumineon":             "457",
		"Tympole":              "535",
		"Terrakion":            "639",
		"Caterpie":             "10",
		"Plusle":               "311",
		"Stunky":               "434",
		"Dewgong":              "87",
		"Treecko":              "252",
		"Klink":                "599",
		"Kirlia":               "281",
		"Nidoran-m":            "32",
		"Marowak":              "105",
		"Quilava":              "156",
		"Lillipup":             "506",
		"Heatmor":              "631",
		"Zweilous":             "634",
		"Pichu":                "172",
		"Corsola":              "222",
		"Druddigon":            "621",
		"Herdier":              "507",
		"Braviary":             "628",
		"Ambipom":              "424",
		"Weavile":              "461",
		"Lickilicky":           "463",
		"Minccino":             "572",
		"Eelektross":           "604",
		"Wormadam-plant":       "413",
		"Panpour":              "515",
		"Garbodor":             "569",
		"Bastiodon":            "411",
		"Floatzel":             "419",
		"Scrafty":              "560",
		"Bagon":                "371",
		"Venipede":             "543",
		"Larvesta":             "636",
		"Dugtrio":              "51",
		"Sudowoodo":            "185",
		"Tyrogue":              "236",
		"Onix":                 "95",
		"Bellossom":            "182",
		"Regice":               "378",
		"Glalie":               "362",
		"Scolipede":            "545",
		"Zangoose":             "335",
		"Rhydon":               "112",
		"Seaking":              "119",
		"Dustox":               "269",
		"Litwick":              "607",
		"Volcarona":            "637",
		"Tentacruel":           "73",
		"Skitty":               "300",
		"Snivy":                "495",
		"Cryogonal":            "615",
		"Tornadus-incarnate":   "641",
		"Seadra":               "117",
		"Registeel":            "379",
		"Sawk":                 "539",
		"Gorebyss":             "368",
		"Cresselia":            "488",
		"Wartortle":            "8",
		"Roselia":              "315",
		"Dusclops":             "356",
		"Luxray":               "405",
		"Moltres":              "146",
		"Gligar":               "207",
		"Nosepass":             "299",
		"Pachirisu":            "417",
		"Boldore":              "525",
		"Pupitar":              "247",
		"Makuhita":             "296",
		"Swoobat":              "528",
		"Zubat":                "41",
		"Kangaskhan":           "115",
		"Aipom":                "190",
		"Hariyama":             "297",
		"Piplup":               "393",
		"Rhyperior":            "464",
		"Stunfisk":             "618",
		"Metapod":              "11",
		"Omastar":              "139",
		"Articuno":             "144",
		"Skuntank":             "435",
		"Probopass":            "476",
		"Ursaring":             "217",
		"Illumise":             "314",
		"Flygon":               "330",
		"Pansage":              "511",
		"Electrode":            "101",
		"Hitmonlee":            "106",
		"Beautifly":            "267",
		"Electrike":            "309",
		"Honchkrow":            "430",
		"Basculin-red-striped": "550",
		"Rhyhorn":              "111",
		"Electabuzz":           "125",
		"Mew":                  "151",
		"Miltank":              "241",
		"Delcatty":             "301",
		"Magnezone":            "462",
		"Dodrio":               "85",
		"Gengar":               "94",
		"Hitmontop":            "237",
		"Scizor":               "212",
		"Mightyena":            "262",
		"Turtwig":              "387",
		"Blitzle":              "522",
		"Porygon":              "137",
		"Aerodactyl":           "142",
		"Politoed":             "186",
		"Gothitelle":           "576",
		"Ferrothorn":           "598",
		"Pidgey":               "16",
		"Machoke":              "67",
		"Serperior":            "497",
		"Swadloon":             "541",
		"Nuzleaf":              "274",
		"Anorith":              "347",
		"Roserade":             "407",
		"Magcargo":             "219",
		"Trapinch":             "328",
		"Tangrowth":            "465",
		"Cottonee":             "546",
		"Maractus":             "556",
		"Butterfree":           "12",
		"Psyduck":              "54",
		"Lickitung":            "108",
		"Simipour":             "516",
		"Zorua":                "570",
		"Vileplume":            "45",
		"Mothim":               "414",
		"Gastrodon":            "423",
		"Spoink":               "325",
		"Darkrai":              "491",
		"Mismagius":            "429",
		"Petilil":              "548",
		"Sigilyph":             "561",
		"Venomoth":             "49",
		"Baltoy":               "343",
		"Walrein":              "365",
		"Cofagrigus":           "563",
		"Pineco":               "204",
		"Salamence":            "373",
		"Glaceon":              "471",
		"Vigoroth":             "288",
		"Lairon":               "305",
		"Klinklang":            "601",
		"Meloetta-aria":        "648",
		"Poliwrath":            "62",
		"Ampharos":             "181",
		"Sunkern":              "191",
		"Lotad":                "270",
		"Swablu":               "333",
		"Haunter":              "93",
		"Pinsir":               "127",
		"Remoraid":             "223",
		"Slowbro":              "80",
		"Weezing":              "110",
		"Starmie":              "121",
		"Spinarak":             "167",
		"Spheal":               "363",
		"Ivysaur":              "2",
		"Squirtle":             "7",
		"Rapidash":             "78",
		"Mantine":              "226",
		"Gulpin":               "316",
		"Drifblim":             "426",
		"Chatot":               "441",
		"Simisage":             "512",
		"Ponyta":               "77",
		"Dratini":              "147",
		"Delibird":             "225",
		"Leavanny":             "542",
		"Aggron":               "306",
		"Meowth":               "52",
		"Chansey":              "113",
		"Kingdra":              "230",
		"Azelf":                "482",
		"Heatran":              "485",
		"Crustle":              "558",
		"Genesect":             "649",
		"Dragonair":            "148",
		"Whismur":              "293",
		"Porygon-z":            "474",
		"Finneon":              "456",
		"Purrloin":             "509",
		"Lampent":              "608",
		"Gloom":                "44",
		"Jynx":                 "124",
		"Hippopotas":           "449",
		"Chinchou":             "170",
		"Grovyle":              "253",
		"Empoleon":             "395",
		"Darmanitan-standard":  "555",
		"Vanillish":            "583",
		"Pidgeotto":            "17",
		"Bellsprout":           "69",
		"Golem":                "76",
		"Buneary":              "427",
		"Froslass":             "478",
		"Mandibuzz":            "630",
		"Magmar":               "126",
		"Omanyte":              "138",
		"Cherubi":              "420",
		"Ledyba":               "165",
		"Ferroseed":            "597",
		"Cradily":              "346",
		"Scyther":              "123",
		"Gyarados":             "130",
		"Donphan":              "232",
		"Eevee":                "133",
		"Bidoof":               "399",
		"Kricketune":           "402",
		"Tauros":               "128",
		"Manectric":            "310",
		"Bibarel":              "400",
		"Skiploom":             "188",
		"Houndoom":             "229",
		"Elekid":               "239",
		"Torchic":              "255",
		"Tepig":                "498",
		"Venusaur":             "3",
		"Rattata":              "19",
		"Chikorita":            "152",
		"Blaziken":             "257",
		"Bonsly":               "438",
		"Muk":                  "89",
		"Ditto":                "132",
		"Celebi":               "251",
		"Marshtomp":            "259",
		"Excadrill":            "530",
		"Swampert":             "260",
		"Bronzong":             "437",
		"Uxie":                 "480",
		"Beedrill":             "15",
		"Mankey":               "56",
		"Kingler":              "99",
		"Espeon":               "196",
		"Vibrava":              "329",
		"Mantyke":              "458",
		"Samurott":             "503",
		"Tirtouga":             "564",
		"Pidgeot":              "18",
		"Cleffa":               "173",
		"Xatu":                 "178",
		"Zoroark":              "571",
		"Escavalier":           "589",
		"Golurk":               "623",
		"Snover":               "459",
		"Happiny":              "440",
		"Cobalion":             "638",
		"Reshiram":             "643",
		"Golduck":              "55",
		"Ninjask":              "291",
		"Clamperl":             "366",
		"Giratina-altered":     "487",
		"Parasect":             "47",
		"Krabby":               "98",
		"Castform":             "351",
		"Poochyena":            "261",
		"Shellos":              "422",
		"Karrablast":           "588",
		"Alomomola":            "594",
		"Nidoran-f":            "29",
		"Poliwag":              "60",
		"Mr-mime":              "122",
		"Simisear":             "514",
		"Rufflet":              "627",
		"Diglett":              "50",
		"Graveler":             "75",
		"Natu":                 "177",
		"Purugly":              "432",
		"Pignite":              "499",
		"Piloswine":            "221",
		"Shedinja":             "292",
		"Burmy":                "412",
		"Grumpig":              "326",
		"Tropius":              "357",
		"Absol":                "359",
		"Buizel":               "418",
		"Azumarill":            "184",
		"Shiftry":              "275",
		"Minun":                "312",
		"Pansear":              "513",
		"Landorus-incarnate":   "645",
		"Shellder":             "90",
		"Milotic":              "350",
		"Grotle":               "388",
		"Dewott":               "502",
		"Sewaddle":             "540",
		"Gothorita":            "575",
		"Quagsire":             "195",
		"Barboach":             "339",
		"Servine":              "496",
		"Sableye":              "302",
		"Chimecho":             "358",
		"Carnivine":            "455",
		"Victini":              "494",
		"Lilligant":            "549",
		"Golbat":               "42",
		"Hitmonchan":           "107",
		"Lanturn":              "171",
		"Bouffalant":           "626",
		"Roggenrola":           "524",
		"Palpitoad":            "536",
		"Sandshrew":            "27",
		"Whiscash":             "340",
		"Mesprit":              "481",
		"Blastoise":            "9",
		"Gabite":               "444",
		"Thundurus-incarnate":  "642",
		"Swanna":               "581",
		"Foongus":              "590",
		"Togepi":               "175",
		"Camerupt":             "323",
		"Liepard":              "510",
		"Joltik":               "595",
		"Porygon2":             "233",
		"Zigzagoon":            "263",
		"Whimsicott":           "547",
		"Lugia":                "249",
		"Loudred":              "294",
		"Grimer":               "88",
		"Tangela":              "114",
		"Wobbuffet":            "202",
		"Chimchar":             "390",
		"Patrat":               "504",
		"Drilbur":              "529",
		"Whirlipede":           "544",
		"Krookodile":           "553",
		"Entei":                "244",
		"Volbeat":              "313",
		"Huntail":              "367",
		"Golett":               "622",
		"Deino":                "633",
		"Virizion":             "640",
		"Koffing":              "109",
		"Taillow":              "276",
		"Watchog":              "505",
		"Groudon":              "383",
		"Mudkip":               "258",
		"Silcoon":              "266",
		"Ralts":                "280",
		"Blissey":              "242",
		"Unfezant":             "521",
		"Zebstrika":            "523",
		"Carracosta":           "565",
		"Tynamo":               "602",
		"Clefairy":             "35",
		"Vaporeon":             "134",
		"Zapdos":               "145",
		"Wynaut":               "360",
		"Phione":               "489",
		"Sandile":              "551",
		"Vanillite":            "582",
		"Fraxure":              "611",
		"Charizard":            "6",
		"Arcanine":             "59",
		"Raikou":               "243",
		"Archen":               "566",
		"Pawniard":             "624",
		"Charmander":           "4",
		"Linoone":              "264",
		"Krokorok":             "552",
		"Staryu":               "120",
		"Riolu":                "447",
		"Exploud":              "295",
		"Torkoal":              "324",
		"Emboar":               "500",
		"Weedle":               "13",
		"Feraligatr":           "160",
		"Sneasel":              "215",
		"Umbreon":              "197",
		"Jirachi":              "385",
		"Darumaka":             "554",
		"Cinccino":             "573",
		"Jellicent":            "593",
		"Ekans":                "23",
		"Vulpix":               "37",
		"Magneton":             "82",
		"Lopunny":              "428",
		"Regigigas":            "486",
		"Haxorus":              "612",
		"Banette":              "354",
		"Sealeo":               "364",
		"Latias":               "380",
		"Croagunk":             "453",
		"Vanilluxe":            "584",
		"Hoothoot":             "163",
		"Mareep":               "179",
		"Chandelure":           "609",
		"Keldeo-ordinary":      "647",
		"Luxio":                "404",
		"Chingling":            "433",
		"Machop":               "66",
		"Suicune":              "245",
		"Solrock":              "338",
		"Ariados":              "168",
		"Dunsparce":            "206",
		"Pikachu":              "25",
		"Jigglypuff":           "39",
		"Goldeen":              "118",
		"Octillery":            "224",
		"Surskit":              "283",
		"Woobat":               "527",
		"Doduo":                "84",
		"Exeggutor":            "103",
		"Marill":               "183",
		"Luvdisc":              "370",
		"Kricketot":            "401",
		"Rampardos":            "409",
		"Abomasnow":            "460",
		"Deerling":             "585",
		"Paras":                "46",
		"Steelix":              "208",
		"Wurmple":              "265",
		"Croconaw":             "159",
		"Misdreavus":           "200",
		"Smoochum":             "238",
		"Shroomish":            "285",
		"Aron":                 "304",
		"Nidorina":             "30",
		"Kadabra":              "64",
		"Kabuto":               "140",
		"Masquerain":           "284",
		"Farfetchd":            "83",
		"Bayleef":              "153",
		"Skarmory":             "227",
		"Jolteon":              "135",
		"Wooper":               "194",
		"Munchlax":             "446",
		"Beheeyem":             "606",
		"Raichu":               "26",
		"Sandslash":            "28",
		"Starly":               "396",
		"Totodile":             "158",
		"Togetic":              "176",
		"Hippowdon":            "450",
		"Wailmer":              "320",
		"Gliscor":              "472",
		"Meganium":             "154",
		"Combusken":            "256",
		"Cascoon":              "268",
		"Lombre":               "271",
		"Budew":                "406",
		"Abra":                 "63",
		"Victreebel":           "71",
		"Drowzee":              "96",
		"Mamoswine":            "473",
		"Stoutland":            "508",
		"Fearow":               "22",
		"Corphish":             "341",
		"Regirock":             "377",
		"Exeggcute":            "102",
		"Latios":               "381",
		"Oshawott":             "501",
		"Bronzor":              "436",
		"Electivire":           "466",
		"Ducklett":             "580",
		"Klang":                "600",
		"Hydreigon":            "635",
		"Machamp":              "68",
		"Sunflora":             "192",
		"Lunatone":             "337",
		"Zekrom":               "644",
		"Houndour":             "228",
		"Seedot":               "273",
		"Slakoth":              "287",
		"Cacturne":             "332",
		"Mime-jr":              "439",
		"Hypno":                "97",
		"Magikarp":             "129",
		"Granbull":             "210",
		"Gigalith":             "526",
		"Archeops":             "567",
		"Horsea":               "116",
	}

	data := GetMonsterByID(pokemons["Hypno"])
	fmt.Println(data.Evol)
	fmt.Println(data.Skim)

}
